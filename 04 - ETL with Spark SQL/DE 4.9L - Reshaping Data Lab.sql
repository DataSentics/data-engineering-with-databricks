...
SELECT * FROM (SELECT user_id user, event_name FROM events) PIVOT (count(*) FOR event_name IN
...
SELECT * FROM events_pivot ep JOIN transactions tr
ON ep.user = tr.user_id
