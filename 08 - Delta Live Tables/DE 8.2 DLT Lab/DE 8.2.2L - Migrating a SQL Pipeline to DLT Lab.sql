...
AS SELECT 
  ...
  CAST(FROM_UNIXTIME(DOUBLE(time), 'yyyy-MM-dd HH:mm:ss') AS TIMESTAMP) time, b.name
...
CREATE OR REFRESH STREAMING LIVE TABLE daily_patient_avg
  COMMENT "comment heartbeat"
AS SELECT mrn,name,mean(heartrate)
AS avg_heartrate, date(time) as date
